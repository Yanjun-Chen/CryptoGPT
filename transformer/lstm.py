...
# Define your decoder-only LSTM model
class DecoderOnlyLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        super(DecoderOnlyLSTM, self).__init__()
    ...
    def forward(self, x):
        ...
        # Pass the output through the fully connected layer to get predictions
        out = self.fc(out)

        return out

# Define your dataset (same as before)

batch_size = 64

# Instantiate the model
model_decoder_only_lstm = DecoderOnlyLSTM(input_size, hidden_size, num_layers, output_size)

# Define loss function and optimizer
criterion = nn.MSELoss()
optimizer = optim.Adam(model_decoder_only_lstm.parameters(), lr=0.001)
...
if __name__ == '__main__':
    ...
    data_path = "/Users/yanjun/code/dev/CryptoGPT/data/" + dataset_name
    wandb_project = "LSTM"
    wandb_run_name = f"{dataset_name}-b:{config.trainer.batch_size}-it:{
        config.trainer.max_iters}-lr:{config.trainer.learning_rate}"
